(() => {
  ...
  const config = {
    ...
    foreground: { speed: 3, maxNumber: 250, scale: 0.4, angle: 0 },
    background: { speed: 1, maxNumber: 750, scale: 0.2, angle: 0 },
  };
  ...
  function createSnow(layer, options) {
    ...
    if (!PIXI || !stage) throw new Error('Run "pixiInit;" before pixiPerform:snow.');
    ...
    const tickerKey = `runtime-snow-${layer}-${++instanceId}`;
  ...
  window.WebGALPixiPerform.register('snow', {
